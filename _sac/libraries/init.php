<?php 
    ...
    if($html->find('header')){
        ...
        foreach($header as $h){
			...
			$html = $html;
            if($html->find('dropdown')){
                $dropdown = $html->find('dropdown');
    ...
    /* Collections */
    if($html->find('.collection')){
		$collection = $html->find('.collection');
		foreach($collection as $c){
			if($c->innertext == ""){
				$c->outertext = $template->collection($c);
			}
        }
		$html = str_get_html($html);
    }
    ...
    if($html->find('.lists')){  
        ...
        foreach($lists as $item){
            if(!trim($item->innertext)){
                if(isset($item->attr['sku'])){
                    $item->outertext = $template->lists($item);
                }
            }
        }
		$html = $html;
